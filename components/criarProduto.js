...
import { storage } from '../service/connectionFirebase'; // Aponte para onde sua configuração do Firebase está
...
const AddProduct = ({ navigation }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      navigation.goBack(); // Navigate back to product list screen (filmes.js)
    } catch (error) {
    ...
    <View style={styles.container}>
      ...
      <View style={{ justifyContent: 'center', alignItems: 'center' }}>
        <View style={{ position: 'relative' }} >
          <Image source={image ? { uri: image } : require('../assets/image_icon.png')} style={{ width: 200, height: 200, marginBottom: 20, backgroundColor: 'white', borderRadius: 15 }} />
